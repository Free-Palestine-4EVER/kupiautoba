...
import { Listing, SearchFilters, Dealer } from '@/types';
...
export async function getListings(
  ...
  // Client-side filtering for ranges (Firestore doesn't allow range on multiple fields with inequality)
...
export async function upgradeToDealer(uid: string, dealerData: Partial<Dealer>): Promise<void> {
  await updateDoc(doc(db, 'users', uid), { isDealer: true });
